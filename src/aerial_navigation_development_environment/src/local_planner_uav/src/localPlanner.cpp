...
string CloudTopic = "/uav/rgbd_camera/depth/points";
string cloudPointFormat = "camera";
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloud2)
{
  ...
  for (int i = 0; i < laserCloudSize; i++) {
    bool inRange = false;
    if (cloudPointFormat == "lidar") {
      float x = laserCloud->points[i].x;
      float y = laserCloud->points[i].y;
      float z = laserCloud->points[i].z;
      inRange = sqrt(x * x + y * y + z * z) < maxRange;
    } else {
      inRange = laserCloud->points[i].z > 0 && laserCloud->points[i].z < maxRange;
    }
  ...
  for (int i = 0; i < laserCloudDwzSize; i++) {
      ...
      if (cloudPointFormat == "lidar") {
        pointX1 = laserCloudDwz->points[i].x;
        pointY1 = laserCloudDwz->points[i].y;
        pointZ1 = laserCloudDwz->points[i].z;
      } else {
        pointX1 = laserCloudDwz->points[i].z;
        pointY1 = -laserCloudDwz->points[i].x;
        pointZ1 = -laserCloudDwz->points[i].y;
      }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("CloudTopic", CloudTopic);
  nhPrivate.param<std::string>("cloudPointFormat", cloudPointFormat, std::string("camera"));
  ...
  if (cloudPointFormat != "camera" && cloudPointFormat != "lidar") {
    ROS_WARN_STREAM("Unknown cloudPointFormat: " << cloudPointFormat << ", reset to camera.");
    cloudPointFormat = "camera";
  }
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> (CloudTopic, 5, laserCloudHandler);

  ros::Subscriber subTrackPoint = nh.subscribe<nav_msgs::Odometry> ("/uav/track_point_odom", 5, trackPointHandler);
  ...
  ros::Subscriber subAutoMode = nh.subscribe<std_msgs::Float32> ("/uav/auto_mode", 5, autoModeHandler);

  ros::Subscriber subClearSurrCloud = nh.subscribe<std_msgs::Empty> ("/uav/clear_surr_cloud", 5, clearSurrCloudHandler);

  ros::Publisher pubPath = nh.advertise<nav_msgs::Path> ("/uav/path", 5);
  nav_msgs::Path path;

  #if PLOTPATHSET == 1
  ros::Publisher pubFreePaths = nh.advertise<sensor_msgs::PointCloud2> ("/uav/free_paths", 2);

  ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2> ("/uav/collision_avoidance_cloud", 2);
  #endif
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      *laserCloudStack[laserCloudCount] = *laserCloudDwz;
      ...
      int lastClearPathCount = 0;
      float lastRelativeGoalX = 0.0;
      float lastRelativeGoalDis = 0.0;
      ...
      while (pathScale >= minPathScale) {
        ...
        lastRelativeGoalX = relativeGoalX;
        lastRelativeGoalDis = relativeGoalDis;
      ...
      if (!pathPublished) {
        ROS_WARN_THROTTLE(
          1.0,
          "UAV local planner has no valid path. planner_cloud=%d clear_paths=%d selected_group=%d rel_goal_x=%.2f rel_goal_dis=%.2f joy=%.2f path_scale=%.2f",
          plannerCloudSize,
          lastClearPathCount,
          lastSelectedGroupID,
          lastRelativeGoalX,
