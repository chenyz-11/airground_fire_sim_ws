...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subOdometry = nh.subscribe<nav_msgs::Odometry> ("/ugv/state_estimation", 5, odometryHandler);

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> ("/ugv/registered_scan", 5, laserCloudHandler);

  ros::Subscriber subRuntime = nh.subscribe<std_msgs::Float32> ("/ugv/runtime", 5, runtimeHandler);

  ros::Publisher pubOverallMap = nh.advertise<sensor_msgs::PointCloud2> ("/ugv/overall_map", 5);

  ros::Publisher pubExploredArea = nh.advertise<sensor_msgs::PointCloud2> ("/ugv/explored_areas", 5);
  pubExploredAreaPtr = &pubExploredArea;

  ros::Publisher pubTrajectory = nh.advertise<sensor_msgs::PointCloud2> ("/ugv/trajectory", 5);
  pubTrajectoryPtr = &pubTrajectory;

  ros::Publisher pubExploredVolume = nh.advertise<std_msgs::Float32> ("/ugv/explored_volume", 5);
  pubExploredVolumePtr = &pubExploredVolume;

  ros::Publisher pubTravelingDis = nh.advertise<std_msgs::Float32> ("/ugv/traveling_distance", 5);
  pubTravelingDisPtr = &pubTravelingDis;

  ros::Publisher pubTimeDuration = nh.advertise<std_msgs::Float32> ("/ugv/time_duration", 5);
  pubTimeDurationPtr = &pubTimeDuration;

  //ros::Publisher pubRuntime = nh.advertise<std_msgs::Float32> ("/ugv/runtime", 5);
