...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subPose = nh.subscribe<nav_msgs::Odometry> ("/ugv/state_estimation", 5, poseHandler);

  ros::Publisher pubWaypoint = nh.advertise<geometry_msgs::PointStamped> ("/ugv/way_point", 5);
  geometry_msgs::PointStamped waypointMsgs;
  waypointMsgs.header.frame_id = "map";

  ros::Publisher pubSpeed = nh.advertise<std_msgs::Float32> ("/ugv/speed", 5);
  std_msgs::Float32 speedMsgs;

  ros::Publisher pubBoundary = nh.advertise<geometry_msgs::PolygonStamped> ("/ugv/navigation_boundary", 5);
