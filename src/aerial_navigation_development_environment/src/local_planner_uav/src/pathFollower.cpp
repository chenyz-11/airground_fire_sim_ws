...
string stateEstimationTopic = "/uav/state_estimation";
...
bool enableStopRotate = true;
bool enableGoalBehindRotate = true;
double goalBehindRotateYaw = 120.0;
double goalBehindRotateMinDis = 2.0;
bool enableSharpTurnRotate = true;
double sharpTurnStartYaw = 65.0;
double sharpTurnEndYaw = 25.0;
double sharpTurnYawBoostScale = 2.0;
double sharpTurnMinGoalDis = 1.0;
double stopDis = 0.5;
double slowDis = 2.0;
...
bool sharpTurnRotateMode = false;
...
void stateEstimationHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  if (autonomyMode) {
    const bool stopRotateTrigger = enableStopRotate
      && odomTime > stopRotTime + minStopRotInterval
      && disToGoal > stopRotDis
      && (fabs(dirToGoal) > stopRotYaw1 * PI / 180.0 ||
          (fabs(dirToGoal) > stopRotYaw2 * PI / 180.0 && vehicleSpeed < minSpeed / 2.0));

    const bool goalBehindRotateTrigger = enableGoalBehindRotate
      && !pathFound
      && odomTime > stopRotTime + minStopRotInterval
      && disToGoal > goalBehindRotateMinDis
      && fabs(dirToGoal) > goalBehindRotateYaw * PI / 180.0;

    if (enableStopRotate || enableGoalBehindRotate) {
      if ((stopRotateTrigger || goalBehindRotateTrigger) && !autoAdjustMode) {
        joyFwd = 0;
        joyLeft = 0;
        joyUp = 0;
        joyYaw = 0;
        ...
        if (goalBehindRotateTrigger) {
          ROS_WARN_THROTTLE(
            1.0,
            "UAV goal is behind and local planner has no path. Rotate in place to face the goal."
          );
        }
      }
  ...
  if (manualMode || (autonomyMode && autoAdjustMode)) {
    ...
    if (joyFwd2 == 0 || !pathFound || (disToGoal < stopDis && autonomyMode && !autoAdjustMode)) {
      if (!pathFound) {
        ROS_WARN_THROTTLE(1.0, "UAV stops because local planner has no valid path.");
      } else if (joyFwd2 == 0) {
        ROS_WARN_THROTTLE(1.0, "UAV stops because forward speed command is zero.");
      } else {
        ROS_WARN_THROTTLE(1.0, "UAV stops because it is within stopDis of the goal.");
      }
    ...
    double sharpTurnStartYawRad = sharpTurnStartYaw * PI / 180.0;
    double sharpTurnEndYawRad = sharpTurnEndYaw * PI / 180.0;
    if (sharpTurnEndYawRad > sharpTurnStartYawRad) {
      sharpTurnEndYawRad = sharpTurnStartYawRad;
    }

    const double absDirDiff = fabs(dirDiff);
    if (enableSharpTurnRotate && autonomyMode && pathFound && disToGoal > sharpTurnMinGoalDis) {
      if (sharpTurnRotateMode) {
        if (absDirDiff < sharpTurnEndYawRad) {
          sharpTurnRotateMode = false;
        }
      } else if (absDirDiff > sharpTurnStartYawRad) {
        sharpTurnRotateMode = true;
      }
    } else {
      sharpTurnRotateMode = false;
    }
    ...
    if (sharpTurnRotateMode) {
      ...
      posXYGain2 = 0;
      yawRatePath = 0;
      yawBoostScale2 = sharpTurnYawBoostScale;
      velZPath = 0;
      posZBoostScale2 = 1.0;

      ROS_INFO_THROTTLE(
        1.0,
        "UAV sharp-turn rotate mode. dir_to_path=%.1f deg",
        dirDiff * 180.0 / PI
      );
    }
    ...
    if (sharpTurnRotateMode) {
      desiredRoll = stopVelXYGain * vehicleVelY;
      if (desiredRoll > maxRollPitch * PI / 180.0) desiredRoll = maxRollPitch * PI / 180.0;
      else if (desiredRoll < -maxRollPitch * PI / 180.0) desiredRoll = -maxRollPitch * PI / 180.0;

      desiredPitch = -stopVelXYGain * vehicleVelX;
      if (desiredPitch > maxRollPitch * PI / 180.0) desiredPitch = maxRollPitch * PI / 180.0;
      else if (desiredPitch < -maxRollPitch * PI / 180.0) desiredPitch = -maxRollPitch * PI / 180.0;
    } else if (dis2 > 0.001) {
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.param<bool>("enableStopRotate", enableStopRotate, true);
  nhPrivate.param<bool>("enableGoalBehindRotate", enableGoalBehindRotate, true);
  nhPrivate.param<double>("goalBehindRotateYaw", goalBehindRotateYaw, 120.0);
  nhPrivate.param<double>("goalBehindRotateMinDis", goalBehindRotateMinDis, 2.0);
  nhPrivate.param<bool>("enableSharpTurnRotate", enableSharpTurnRotate, true);
  nhPrivate.param<double>("sharpTurnStartYaw", sharpTurnStartYaw, 65.0);
  nhPrivate.param<double>("sharpTurnEndYaw", sharpTurnEndYaw, 25.0);
  nhPrivate.param<double>("sharpTurnYawBoostScale", sharpTurnYawBoostScale, 2.0);
  nhPrivate.param<double>("sharpTurnMinGoalDis", sharpTurnMinGoalDis, 1.0);
  ...
  ros::Subscriber subPath = nh.subscribe<nav_msgs::Path> ("/uav/path", 5, pathHandler);

  ros::Subscriber subJoystick = nh.subscribe<sensor_msgs::Joy> ("/uav/joy", 5, joystickHandler);

  ros::Subscriber subGoal = nh.subscribe<geometry_msgs::PointStamped> ("/uav/way_point", 5, goalHandler);

  ros::Subscriber subSpeed = nh.subscribe<std_msgs::Float32> ("/uav/speed", 5, speedHandler);

  ros::Publisher pubMarker = nh.advertise<visualization_msgs::Marker> ("/uav/track_point_marker", 5);
  pubMarkerPointer = &pubMarker;

  ros::Publisher pubOdometry = nh.advertise<nav_msgs::Odometry> ("/uav/track_point_odom", 5);
  pubOdometryPointer = &pubOdometry;

  ros::Publisher pubPath = nh.advertise<nav_msgs::Path> ("/uav/track_path", 5);
  pubPathPointer = &pubPath;

  ros::Publisher pubControl = nh.advertise<geometry_msgs::TwistStamped> ("/uav/attitude_control", 5);
  pubControlPointer = &pubControl;

  ros::Publisher pubAutoMode = nh.advertise<std_msgs::Float32> ("/uav/auto_mode", 5);
  pubAutoModePointer = &pubAutoMode;
  ros::Publisher pubWaypoint = nh.advertise<geometry_msgs::PointStamped> ("/uav/way_point", 5);
