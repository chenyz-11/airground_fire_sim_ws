...
string robotModelName = "robot";
string lidarModelName = "lidar";
string cameraModelName = "camera";
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("robotModelName", robotModelName);
  nhPrivate.getParam("lidarModelName", lidarModelName);
  nhPrivate.getParam("cameraModelName", cameraModelName);

  ros::Subscriber subScan = nh.subscribe<sensor_msgs::PointCloud2>("/ugv/velodyne_points", 2, scanHandler);

  ros::Subscriber subTerrainCloud = nh.subscribe<sensor_msgs::PointCloud2>("/ugv/terrain_map", 2, terrainCloudHandler);

  ros::Subscriber subSpeed = nh.subscribe<geometry_msgs::TwistStamped>("/ugv/cmd_vel", 5, speedHandler);

  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry>("/ugv/state_estimation", 5);
  ...
  cameraState.model_name = cameraModelName;
  gazebo_msgs::ModelState lidarState;
  lidarState.model_name = lidarModelName;
  gazebo_msgs::ModelState robotState;
  robotState.model_name = robotModelName;

  ros::Publisher pubScan = nh.advertise<sensor_msgs::PointCloud2>("/ugv/registered_scan", 2);
