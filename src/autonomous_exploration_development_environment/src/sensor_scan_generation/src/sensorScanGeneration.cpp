...
int main(int argc, char** argv)
{
  ...
  subOdometry.subscribe(nh, "/ugv/state_estimation", 1);
  subLaserCloud.subscribe(nh, "/ugv/registered_scan", 1);
  ...
  ros::Publisher pubOdometry = nh.advertise<nav_msgs::Odometry> ("/ugv/state_estimation_at_scan", 5);
  pubOdometryPointer = &pubOdometry;

  tf::TransformBroadcaster tfBroadcaster;
  tfBroadcasterPointer = &tfBroadcaster;

  pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2>("/ugv/sensor_scan", 2);
