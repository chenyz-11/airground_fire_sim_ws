...
string lidarModelName = "uav_lidar";
string rgbdCameraModelName = "rgbd_camera";
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.param<std::string>("lidarModelName", lidarModelName);
  nhPrivate.getParam("rgbdCameraModelName", rgbdCameraModelName);

  ros::Subscriber subControl = nh.subscribe<geometry_msgs::TwistStamped> ("/uav/attitude_control", 5, controlHandler);

  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry> ("/uav/state_estimation", 5);

  nav_msgs::Odometry odomData;
  odomData.header.frame_id = "map";
  odomData.child_frame_id = "uav_vehicle";

  tf::TransformBroadcaster tfBroadcaster;
  tf::StampedTransform odomTrans;
  odomTrans.frame_id_ = "map";
  odomTrans.child_frame_id_ = "uav_vehicle";
  ...
  cameraState.model_name = rgbdCameraModelName;
  ...
  while (status) {
    ...
    if (timeNow <= odomTimeLast) {
      timeNow = odomTimeLast + ros::Duration(0.005);
    }
    odomTimeLast = timeNow;
